...
type Concern interface {
	...
	// Add 添加一个订阅
	Add(ctx mmsg.IMsgCtx, target mt.Target, id interface{}, ctype concern_type.Type) (IdentityInfo, error)
	// Remove 删除一个订阅
	Remove(ctx mmsg.IMsgCtx, target mt.Target, id interface{}, ctype concern_type.Type) (IdentityInfo, error)
	// Get 获取一个订阅信息
